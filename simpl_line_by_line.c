...
int copy_file(char *input_filename, char *output_filename) {
    ...
    char *input_string = (char *) malloc(buff_size);

    while ((bytes_read = getline(&input_string, &buff_size,(size_t) input_file)) != -1) {
        size_t bytes_written = fwrite(input_file, sizeof(char), strlen(input_string), output_file);
        if (bytes_read != bytes_written) {
            perror("Unable to write all read bytes");
            break;
        }
    }
